            ...
            if (member.status != 'left' && member.status != 'kicked') {
                ...
                fetch(file_link, { headers: headers })
                    ...
                    .then(async (blob) => {
                        ...
                        bot.sendMessage(ctx.chat.id, txt, {
                            parse_mode: 'Markdown'
                        })
                            .catch(() => { bot.sendMessage(msg.chat.id, txt) })
                    })
                    .catch(() => { bot.sendMessage(ctx.chat.id, 'عذرا حدث خطأ ما ربما بسبب الأنترنيت, حاول مجددا, او حاول كتابت سؤالك بصيغه اخرا') })
                    ...
                    if (member.status != 'left' && member.status != 'kicked') {
                        ...
                        fetch(file_link, { headers: headers })
                            ...
                            .then(async (blob) => {
                                ...
                                // console.log("_______rest______img_", result.response)
                                const txt = result.response.text()
                                bot.sendMessage(ctx.chat.id, txt, {
                                    parse_mode: 'Markdown'
                                })
            ...
            bot.getChatMember('@bashar_prog', msg.chat.id).then(async (member) => {
                if (member.status != 'left' && member.status != 'kicked') {
                    ...
                    const result = await chat.sendMessage(msg.text)
                        .catch(() => { bot.sendMessage(msg.chat.id, 'عذرا حدث خطأ ما ربما بسبب الأنترنيت, حاول مجددا, او حاول كتابت سؤالك بصيغه اخرا') })
                    // console.log("_______rest______4_", await result.response.text())
                    // console.log("_______rest______img_", result.response)
                    const txt = await result.response.text()
                    bot.sendMessage(msg.chat.id, txt, {
                        parse_mode: 'Markdown'
                    })
                        .catch(async () => { bot.sendMessage(msg.chat.id, txt) })
                } else if (member.status == 'kicked') {
